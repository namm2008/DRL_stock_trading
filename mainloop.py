...
# -*- coding: utf-8 -*-
"""
Created on Thu Jan 14 18:03:10 2021
...
from attention_gru import GRU_Attention
from attention_lstm import LSTM_Attention
import Backtest
import Optimization
            ...
            for data_time_data in data_time_data_list:
                ...
                for i_episode in range(num_episodes):
                    ...
                    for t in range(iterate_from, iterate_end):
                        ...
                        a = select_action(state, en_hidden_test, de_hidden_test)
                        ...
                        if len(memory) == 0 or memory.last_ls() == 0:
                            if a == 0:
                                reward = reward_action_train(df_train['price'], a, t, timestep, cost_percent)
                ...
                for i_episode in range(num_episodes):
                    ...
                    for t in range(iterate_from, iterate_end):
                        ...
                        # Select and perform an action
                        a,a0,a1,a2 = select_action_val(state, en_hidden_test, de_hidden_test)
                        en_hidden = en_hidden_1 = policy_net.init_hidden_encoder(BATCH_SIZE)
                        de_hidden = de_hidden_1 = policy_net.init_hidden_decoder(BATCH_SIZE)
                        ...
                        if len(memory) == 0 or memory.last_ls() == 0:
                            if a == 0:
                                reward = reward_action_train(df_test['price'], a, t, timestep, cost_percent)
                                ls = torch.tensor(0,device=device, dtype=torch.int64)
                                memory.push(Transition(state, a.view(1,1), next_state, reward.view(1,1), ls))
                            elif a == 1:
                                reward = reward_action_train(df_test['price'], a, t, timestep, cost_percent)
